 ...
 *
 * Collections are auto-created on first use — no manual init script needed.
 */
...
/* ------------------------------------------------------------------ */
/*  Auto-init: lazily create collections on first use                 */
/* ------------------------------------------------------------------ */

const _collectionReady = {} // { collectionName: true }
...
const COLLECTION_SCHEMAS = {
  inquiries: {
    ...
    fields: [
      ...
      { name: "status", type: "select", required: true, values: ["new", "triaged", "responded", "closed"], maxSelect: 1 },
...
/**
 * Ensure a collection exists in PocketBase, creating it if missing.
 * Safe to call repeatedly — only hits the API once per collection per process.
 */
async function ensureCollection(name) {
  if (_collectionReady[name]) return
  const token = await getAdminToken()
  if (!token) return // can't auth — will fail downstream anyway
  const base = getBase()

  // Quick check: does the collection already exist?
  const checkRes = await fetch(`${base}/api/collections/${name}`, {
    headers: { Authorization: token },
  })
  if (checkRes.ok) {
    _collectionReady[name] = true
    return
  }
  ...
  if (!schema) {
    console.warn(`[PocketBase] No schema defined for collection "${name}", skipping auto-init.`)
    return
  }

  console.log(`[PocketBase] Auto-creating collection "${name}"...`)
  ...
  if (createRes.ok) {
    console.log(`[PocketBase] Collection "${name}" created successfully.`)
    _collectionReady[name] = true
  } else {
    const err = await createRes.json().catch(() => ({}))
    // Already exists (race condition between check and create) — that's fine
    if (err.message?.includes("already exists") || err.data?.name?.message?.includes("already exists")) {
      _collectionReady[name] = true
    } else {
      console.error(`[PocketBase] Failed to auto-create collection "${name}":`, createRes.status, JSON.stringify(err))
    }
  }
}
...
export async function createScorecardSubmission(data) {
  await ensureCollection("scorecard_submissions")
  const token = await getAdminToken()
  if (!token) return null
  const base = getBase()
  const tierMap = {
...
export async function createScorecardInquiry(data) {
  await ensureCollection("scorecard_inquiries")
  const token = await getAdminToken()
  if (!token) return null
  const base = getBase()
  const tierMap = {
...
export async function getStats() {
  await Promise.all([ensureCollection("inquiries"), ensureCollection("scorecard_submissions")])
  const token = await getAdminToken()
